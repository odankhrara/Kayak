#!/usr/bin/env python3
"""
Load Flight Price Prediction dataset into the system

Dataset: https://www.kaggle.com/datasets/shubhambathwal/flight-price-prediction

This script:
1. Reads the Flight Price Prediction CSV file
...
async def load_to_kafka(csv_path: str):
    ...
    await loader.load_flight_price_dataset(csv_path)
...
def load_directly_to_db(csv_path: str, limit: int = 100):
    ...
    # Process CSV
    records = CSVProcessor.process_dataset(csv_path, "flight_price")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Load Flight Price Prediction dataset")
    parser.add_argument(
        "csv_path",
        nargs="?",
        default="data/raw/flight_prices.csv",
        help="Path to Flight Price CSV file (default: data/raw/flight_prices.csv)"
    )
    parser.add_argument(
        "--method",
