#!/usr/bin/env python3
"""
Load Hotel Booking Demand dataset into the system

Dataset: https://www.kaggle.com/datasets/mojtaba142/hotel-booking

This script:
1. Reads the Hotel Booking Demand CSV file
...
async def load_to_kafka(csv_path: str):
    ...
    await loader.load_hotel_booking_dataset(csv_path)
...
def load_directly_to_db(csv_path: str, limit: int = 100):
    ...
    # Process CSV
    records = CSVProcessor.process_dataset(csv_path, "hotel_booking")
        ...
        for record in records:
            ...
            if deal_info.get("is_good_deal") or True:  # Create all for testing
                ...
                hotel_deal = HotelDeal(
                    name=hotel_name[:100],  # Limit length
                    city=city,
                    state=None,  # Hotel booking dataset may not have state
                    country=record.get("country", "Unknown"),
                    address=f"{city}, {record.get('country', '')}",
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Load Hotel Booking Demand dataset")
    parser.add_argument(
        "csv_path",
        nargs="?",
        default="data/raw/hotel_booking.csv",
        help="Path to Hotel Booking CSV file (default: data/raw/hotel_booking.csv)"
    )
    parser.add_argument(
        "--method",
    ...
    if not csv_path.exists():
        ...
        print("   3. Run: ./scripts/download_all_datasets.sh")
        print("\n   Or download manually from:")
        print("   https://www.kaggle.com/datasets/mojtaba142/hotel-booking")
        sys.exit(1)
