"""Ollama Service - Integration with Ollama for LLM-powered responses"""
import httpx
...
class OllamaService:
    """
    Service for interacting with Ollama (local LLM runner)
    
    Ollama allows running large language models locally without API keys.
    ...
    def __init__(self, base_url: str = "http://localhost:11434", model: str = "llama3.2"):
        ...
        Args:
            ...
            model: Model name to use (default: llama3.2)
                   Options: llama3.2, mistral, codellama, phi3, etc.
        """
        ...
        if CSV_QUERY_AVAILABLE:
            try:
                ...
                print(f"[OllamaService] CSV query service not available: {e}")
                self.csv_query = None
        else:
            self.csv_query = None
    ...
    def generate(
        ...
        try:
            # Use official ollama package if available
    ...
    def parse_trip_request(self, message: str) -> Dict[str, Any]:
        """
        Use Ollama to parse natural language trip requests with CSV data context
...
Respond in JSON format:
{
  ...
  "confidence": 0.85
        ...
        try:
            ...
            print(f"[OllamaService] Failed to parse JSON: {e}")
            return {
                "origin": None,
                "destination": None,
                "city": None,
...
Hotels: {len(hotels)} hotel(s)"""
    ...
    def answer_policy_question(
        ...
        """
        Answer policy questions using Ollama with context and CSV data
...
{context_str}"""
    ...
    def _fallback_response(self, prompt: str) -> str:
        ...
        return "I'm currently using rule-based responses. To enable AI-powered responses, please install and start Ollama (https://ollama.ai)."
