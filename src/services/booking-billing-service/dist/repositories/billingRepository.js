...
class BillingRepository {
     ...
     */
    async create(billingData, connection) {
        const conn = connection || mysqlPool_1.default;
        await conn.execute(`INSERT INTO billing (
    ...
    async getById(billingId) {
        const [rows] = await mysqlPool_1.default.execute(`SELECT b.*, u.email as user_email, u.first_name, u.last_name
       FROM billing b
       LEFT JOIN users u ON b.user_id = u.user_id
       WHERE b.billing_id = ?`, [billingId]);
        if (rows.length === 0)
            return null;
        return this.mapRowToBilling(rows[0]);
    }
    /**
     * Get billing by booking ID
     */
    async getByBookingId(bookingId) {
        const [rows] = await mysqlPool_1.default.execute('SELECT * FROM billing WHERE booking_id = ?', [bookingId]);
        if (rows.length === 0)
            return null;
        return this.mapRowToBilling(rows[0]);
    }
    ...
    async getByUserId(userId) {
        const [rows] = await mysqlPool_1.default.execute('SELECT * FROM billing WHERE user_id = ? ORDER BY transaction_date DESC', [userId]);
        return rows.map(row => this.mapRowToBilling(row));
    }
    /**
     * Update billing/transaction status
     */
    async updateStatus(billingId, status, connection) {
        const conn = connection || mysqlPool_1.default;
        await conn.execute('UPDATE billing SET transaction_status = ? WHERE billing_id = ?', [status, billingId]);
    }
    /**
    ...
    async getRevenueByPeriod(startDate, endDate) {
        ...
        const [totals] = await mysqlPool_1.default.execute(`SELECT 
         COUNT(*) as total_transactions,
         SUM(total_amount) as total_revenue
       FROM billing
       WHERE DATE(transaction_date) BETWEEN ? AND ?`, [startDate, endDate]);
        // Revenue by booking type
        const [byType] = await mysqlPool_1.default.execute(`SELECT 
         booking_type,
         COUNT(*) as count,
         SUM(total_amount) as revenue
       FROM billing
       WHERE DATE(transaction_date) BETWEEN ? AND ?
       GROUP BY booking_type`, [startDate, endDate]);
        // Revenue by status
        const [byStatus] = await mysqlPool_1.default.execute(`SELECT 
         transaction_status as status,
         COUNT(*) as count,
         SUM(total_amount) as revenue
       FROM billing
       WHERE DATE(transaction_date) BETWEEN ? AND ?
       GROUP BY transaction_status`, [startDate, endDate]);
        return {
     ...
     */
    async getTopPropertiesByRevenue(year, limit = 10) {
        const [rows] = await mysqlPool_1.default.execute(`SELECT 
         b.booking_type,
         b.booking_id,
         CASE 
    ...
    async getAll(limit = 100) {
        const [rows] = await mysqlPool_1.default.execute(`SELECT b.*, u.email as user_email 
       FROM billing b
       LEFT JOIN users u ON b.user_id = u.user_id
