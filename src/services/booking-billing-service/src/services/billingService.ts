...
export class BillingService {
   ...
   */
  async getById(billingId: string): Promise<any> {
   ...
   */
  async getUserBillings(userId: string): Promise<any[]> {
  ...
  async searchBillings(filters: {
    ...
    paymentMethod?: 'credit_card' | 'debit_card' | 'paypal' | 'refund';
    startDate?: string;
    endDate?: string;
    minAmount?: number;
    maxAmount?: number;
   ...
   */
  async getTopProperties(year: number, limit: number = 10): Promise<Array<{
    entityId: string;
    bookingType: string;
    revenue: number;
    bookings: number;
    ...
    try {
      ...
      const propertyRevenue: {
        [key: string]: { revenue: number; bookings: number; type: string }
      } = {}
   ...
   */
  async getCityRevenue(year: number): Promise<Array<{
    city: string;
    revenue: number;
    bookings: number;
  }>> {
   ...
   */
  async generateInvoice(billingId: string): Promise<{
   ...
   */
  async getAllBillings(limit: number = 100): Promise<any[]> {
