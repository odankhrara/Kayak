...
export class BookingService {
   ...
   */
  private generateBookingRef(type: string): string {
   ...
   */
  private validateCreditCard(cardNumber: string, cvv: string, expiryDate: string): void {
  ...
  async createBookingWithPayment(bookingData: {
    userId: string;
    bookingType: 'flight' | 'hotel' | 'car';
    entityId: string; // flight_id, hotel_id, car_id
    quantity: number; // seats, rooms, 1 for car
    checkInDate: string;
    checkOutDate: string;
    totalAmount: number;
   ...
   */
  async getUserBookings(userId: string, filter?: 'past' | 'current' | 'future'): Promise<any[]> {
   ...
   */
  async cancelBooking(bookingId: string, userId: string, reason?: string): Promise<{
    message: string;
    refundAmount: number;
  }> {
    const connection = await mysqlPool.getConnection()
   ...
   */
  async updateStatus(bookingId: string, status: 'pending' | 'confirmed' | 'cancelled' | 'completed'): Promise<any> {
    const booking = await this.bookingRepository.getById(bookingId)
    if (!booking) {
      throw new Error(`Booking ${bookingId} not found`)
    }
   ...
   */
  async getAllBookings(limit: number = 100): Promise<any[]> {
