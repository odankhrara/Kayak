...
export class CarService {
   ...
   * Get car by ID
   */
  async getById(carId: string): Promise<any> {
   ...
   */
  async updateCar(carId: string, updates: any): Promise<any> {
    const car = await this.repository.getById(carId)
    if (!car) {
      throw new Error(`Car ${carId} not found`)
    }
   ...
   */
  async checkAvailability(carId: string): Promise<{
    available: boolean;
    carId: string;
  }> {
    const car = await this.repository.getById(carId)
   ...
   */
  async getAllCars(limit: number = 100): Promise<any[]> {
   ...
   */
  calculateRentalCost(dailyRate: number, pickupDate: string, returnDate: string): {
    days: number;
    dailyRate: number;
    subtotal: number;
    tax: number;
