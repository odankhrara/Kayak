...
class HotelService {
    constructor() {
        this.repository = new hotelRepository_1.HotelRepository();
    }
    /**
    ...
    async search(filters) {
        try {
            ...
            // Try cache first
            const cachedHotels = await redisCache_1.redisCache.get(cacheKey);
            if (cachedHotels) {
                return cachedHotels;
            }
            ...
            await redisCache_1.redisCache.set(cacheKey, hotels, 300);
            return hotels;
        }
        catch (error) {
            throw error;
    ...
    async getById(hotelId) {
        ...
        const cachedHotel = await redisCache_1.redisCache.get(cacheKey);
        if (cachedHotel) {
            return cachedHotel;
        }
        // Cache miss - query database
        ...
        await redisCache_1.redisCache.set(cacheKey, hotel, 600);
        return hotel;
    }
    /**
    ...
    async updateHotel(hotelId, updates) {
        ...
        // Invalidate caches
        await redisCache_1.redisCache.del(`hotel:${hotelId}`);
        await redisCache_1.redisCache.delPattern('hotels:search:*');
     ...
     */
    async checkAvailability(hotelId, roomType, roomCount) {
        const hotel = await this.repository.getById(hotelId);
        if (!hotel) {
            throw new Error(`Hotel ${hotelId} not found`);
        }
