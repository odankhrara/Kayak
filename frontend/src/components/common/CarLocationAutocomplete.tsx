...
import { MapPin, Search, X, Car } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { carService, CarLocation } from '../../services/car.service';
...
export default function CarLocationAutocomplete({
  value,
  onChange,
  ...
  useEffect(() => {
    const fetchLocations = async () => {
      setIsLoading(true);
      try {
        const data = await carService.getLocations(inputValue || undefined);
        setLocations(data);
      } catch (error) {
  ...
  const handleSelect = (location: CarLocation) => {
                ...
                {locations.map((location, index) => (
                  ...
                  >
                    ...
                    <span className="text-xs text-slate-400 bg-slate-100 dark:bg-slate-700 px-2 py-1 rounded-full">
                      {location.carCount} car{location.carCount !== 1 ? 's' : ''}
                    </span>
                  </li>
                ))}
              </ul>
